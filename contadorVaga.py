...
while True:
    ...
    ## Dilata a imagem para expandir onde tem maior quantidade de cor
    kernel = np.ones((3,3),np.int8)
    imgDil = cv2.dilate(imgBlur,kernel)

    for x,y,w,h in vagas:
        recorte = imgDil[y:y+h,x:x+w]
        qntPxBranco = cv2.countNonZero(recorte)
        cv2.putText(img,str(qntPxBranco),(x,y+h-10), cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,255),1)
